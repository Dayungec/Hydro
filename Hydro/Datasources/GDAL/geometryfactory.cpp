...
OGRMultiLineString*GDALGeometryFactory::ConvertPolyline(Polyline*pDef)
{
    ...
    for(long k=0;k<Count;k++)
    {
        ...
        part->setPoints(lCount,rawPoints,zs);
...
OGRGeometry*GDALGeometryFactory::ConvertGeometry(Geometry*pDef)
{
    ...
    case gtEnvelope:
        {
            AutoPtr<Envelope>rt=AutoPtr<Geometry>(pDef,true);
    ...
    case gtEllipse:
        {
           AutoPtr<Ellipse>rt=AutoPtr<Geometry>(pDef,true);
    ...
    case gtSegment:
        {
            AutoPtr<Segment>seg=AutoPtr<Geometry>(pDef,true);
    ...
    case gtPolyline:
        {
            AutoPtr<Polyline>poly=AutoPtr<Geometry>(pDef,true);
            return ConvertPolyline(poly.get());
        }
    case gtPolygon:
        {
            AutoPtr<Polygon>poly=AutoPtr<Geometry>(pDef,true);
            return ConvertPolygon(poly.get());
        }
    case gtMultiPolygon:
        {
            AutoPtr<MultiPolygon>poly=AutoPtr<Geometry>(pDef,true);
            return ConvertMultiPolygon(poly.get());
        }
    case gtCollection:
        {
            AutoPtr<GeometryCollection>poly=AutoPtr<Geometry>(pDef,true);
            return ConvertGeometryCollection(poly.get());
        }
    }
    return NULL;
}
OGRGeometry* GDALGeometryFactory::ConvertPoint(DOUBLE X,DOUBLE Y,DOUBLE Z)
{
    OGRPoint*def=(OGRPoint*)OGRGeometryFactory::createGeometry(wkbPoint);
    def->setX(X);
    def->setY(Y);
...
AutoPtr<Geometry>GeometryFactory::CreatePoint(DOUBLE X,DOUBLE Y,DOUBLE Z)
{
    Point*ppt=new Point();
    ppt->PutCoord(X,Y,Z);
    return ppt;
