...
class SGIS_EXPORT GeometryCollection:
        public Geometry,
        public Geometry3D
{
public:
...
//Polyline:
    int GetSize();
    AutoPtr<Geometry>operator[](int nIndex);
    AutoPtr<Geometry>GetItem(int nIndex);
    void Add(AutoPtr<Geometry>item);
    void Insert(int nIndex,AutoPtr<Geometry>item);
    void Clear();
    void Remove(int nIndex);
protected:
    vector<AutoPtr<Geometry>>parts;
};
